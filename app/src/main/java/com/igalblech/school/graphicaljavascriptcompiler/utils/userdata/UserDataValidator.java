...
public class UserDataValidator {
    ...
    public static final int MAX_PASSWORD = 24;
    public static final int MIN_USERNAME = 8;
    public static final int MAX_USERNAME = 24;
    ...
    public static String errorToString(int v) {
        ...
        switch (v) {
            case ERROR_NONE:
                return String.format(Locale.US, "%s is okay!", s);
            case ERROR_EMPTY:
                return String.format(Locale.US, "%s is empty!", s);
            case ERROR_PASSWORD_LENGTH:
                return String.format(Locale.US, "%s should have %d to %d characters!", s, MIN_PASSWORD, MAX_PASSWORD);
            case ERROR_NO_LETTER:
                return String.format(Locale.US, "%s should have at least one letter!", s);
            case ERROR_NO_NUMBER:
                return String.format(Locale.US, "%s should have at least one number!", s);
            case ERROR_NO_AT_SING:
                return String.format(Locale.US, "%s missing at sing (@) !", s);
            case ERROR_BAD_FORMATTING:
                return String.format(Locale.US, "%s has invalid/bad formatting!", s);
            case ERROR_USERNAME_LENGTH:
                return  String.format(Locale.US, "%s should have %d to %d characters!", s, MIN_USERNAME, MAX_USERNAME);
            case ERROR_PHONE_LENGTH:
                return  String.format(Locale.US, "%s should have 10 numbers!", s);
            case ERROR_NO_SPACES:
                return  String.format(Locale.US, "%s shouldn't have any spaces!", s);
            case ERROR_ONLY_NUMBER:
                return  String.format(Locale.US, "%s should only have numbers!", s);
            ...
            case ERROR_EMAIL_LENGTH:
                return String.format(Locale.US, "%s should have less than %d characters!", s, MAX_EMAIL);
                default:
                    return String.format(Locale.US, "%s have an unknown error!", s);
        }
    }
